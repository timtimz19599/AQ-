...
import { Clock, CalendarDays, TrendingUp, TrendingDown, Minus, Bell, Pencil, LogOut } from 'lucide-react';
...
export function SchedulePage() {
  ...
  const weeks = useMonthGrid(year, month, filterUsername);
  ...
  return (
    <div className="h-screen bg-[#f8fafc] flex overflow-hidden">
      {/* Sidebar */}
      <aside className="w-60 shrink-0 bg-white border-r border-[#e2e8f0] flex flex-col shadow-sm h-screen">

        {/* Header */}
        <div className="bg-gradient-to-b from-[#1e3a5f] to-[#162d4a] px-4 py-4">
          <div className="flex items-center gap-2 mb-1">
            <div className="bg-white rounded-lg px-1.5 py-0.5 shrink-0">
              <img src="/logo.png" alt="AQ" className="h-7 object-contain" />
            </div>
            <span className="text-white font-bold text-sm leading-tight">AQ 课程管理</span>
          </div>
          <div className="h-0.5 bg-[#f59e0b] rounded-full mt-2 w-8" />
          {session && (
            <div className="mt-2 flex items-center gap-2">
              {teacherStats && (
                <span className="w-2.5 h-2.5 rounded-full shrink-0 ring-2 ring-white/30"
                  style={{ backgroundColor: teacherStats.color }} />
              )}
              <div className="flex-1 min-w-0">
                <div className="text-white/90 text-xs font-semibold truncate">{session.displayName}</div>
                <div className="text-white/45 text-[10px]">{session.role === 'admin' ? '管理员' : '教师'}</div>
              </div>
        ...
        {session?.role === 'admin' && (
          <div className="px-3 py-2 border-b border-[#e2e8f0]">
            <Link to="/admin" className="text-xs text-[#1e3a5f] hover:underline font-medium">管理面板</Link>
          </div>
        )}
          ...
          <div className="flex-1 overflow-y-auto flex flex-col gap-3 p-3">
            ...
            <div className="bg-[#f8fafc] rounded-xl p-3 border border-[#e2e8f0]">
              <div className="flex items-center gap-1.5 mb-2">
                ...
                <span className="text-[10px] font-semibold text-[#1e3a5f] uppercase tracking-wide">
              ...
              <div className="flex justify-between mb-2">
                <div className="flex flex-col">
                  <span className="text-[10px] text-[#64748b]">已授课</span>
                  <span className="text-lg font-bold text-[#0f172a] leading-tight">{teacherStats.taughtH}<span className="text-xs font-normal text-[#64748b] ml-0.5">h</span></span>
                </div>
                <div className="w-px bg-[#e2e8f0]" />
                <div className="flex flex-col items-end">
                  <span className="text-[10px] text-[#64748b]">待授课</span>
                  <span className="text-lg font-bold text-[#f59e0b] leading-tight">{teacherStats.remainingH}<span className="text-xs font-normal text-[#64748b] ml-0.5">h</span></span>
                </div>
              </div>
              ...
              <div className="flex justify-between mt-1">
                <span className="text-[9px] text-[#64748b]">已上 {teacherStats.taughtPct.toFixed(0)}%</span>
                <span className="text-[9px] text-[#64748b]">共 {teacherStats.thisMonthH}h</span>
              </div>
            </div>
            ...
            <div className="bg-[#f8fafc] rounded-xl p-3 border border-[#e2e8f0]">
              <div className="flex items-center gap-1.5 mb-2">
                ...
                <span className="text-[10px] font-semibold text-[#1e3a5f] uppercase tracking-wide">下次课程</span>
              </div>
              {teacherStats.next ? (
                <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-1.5">
                    <CalendarDays className="w-3 h-3 text-[#64748b] shrink-0" />
                    <span className="text-xs font-semibold text-[#0f172a]">{teacherStats.next.date}</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <Clock className="w-3 h-3 text-[#64748b] shrink-0" />
                    <span className="text-xs text-[#0f172a]">{teacherStats.next.startTime} – {teacherStats.next.endTime}</span>
                  </div>
                  <div className="mt-1 bg-white rounded-lg px-2 py-1.5 border border-[#e2e8f0]">
                    <div className="text-[10px] text-[#64748b] truncate">{teacherStats.next.teamName}</div>
                    <div className="text-xs font-medium text-[#0f172a] truncate">{teacherStats.next.courseName}</div>
                  </div>
                </div>
              ) : (
                <p className="text-xs text-[#64748b]">暂无待上课程</p>
              )}
            </div>
            ...
            <div className="bg-[#f8fafc] rounded-xl p-3 border border-[#e2e8f0]">
              <div className="flex items-center gap-1.5 mb-2">
                ...
                <span className="text-[10px] font-semibold text-[#1e3a5f] uppercase tracking-wide">累计统计</span>
              </div>
              <div className="flex flex-col gap-1.5">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] text-[#64748b]">累计课时</span>
                  <span className="text-sm font-bold text-[#0f172a]">{teacherStats.totalH}h</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-[10px] text-[#64748b]">上月课时</span>
                  <span className="text-sm text-[#64748b]">{teacherStats.lastMonthH}h</span>
                </div>
                <div className="h-px bg-[#e2e8f0]" />
                <div className="flex items-center justify-between">
                  <span className="text-[10px] text-[#64748b]">较上月</span>
                  <div className="flex items-center gap-1">
                    {teacherStats.diffSign === '+' && <TrendingUp className="w-3 h-3 text-emerald-500" />}
                    {teacherStats.diffSign === '-' && <TrendingDown className="w-3 h-3 text-red-400" />}
                    {teacherStats.diffSign === '=' && <Minus className="w-3 h-3 text-[#64748b]" />}
